 ...
 * The site is currently served over http://<ip>, where it is `undefined` and
...
export function uuid(): string {
  ...
  if (c && typeof c.getRandomValues === "function") {
    ...
    // Ancient browser — not cryptographically strong, but the id only needs to
    // be unique per tab for an anonymous "online now" counter.
