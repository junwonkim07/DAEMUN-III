 ...
 * (TextBoxReveal). Same sticky 400vh mechanism; the difference is that a
 * whole phrase (not just one word) can be highlighted, and the copy is
 * passed in as paragraphs rather than parsed from JSX.
 */
  ...
  return (
    <div ref={containerRef} className="relative z-0 h-[300vh]">
      <div className="sticky top-0 mx-auto flex h-screen max-w-4xl items-center px-5 py-20 sm:px-8">
        <div className={cn("flex flex-col gap-10 text-2xl leading-[1.35] tracking-tight md:text-3xl lg:text-4xl", className)}>
          {lines.map((words, li) => {
            const start = offset;
            offset += words.length;
            return (
...
function AnimatedWord({
  ...
  // skiper70 timing: each word fades in over a window that overlaps the next 15
